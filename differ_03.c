...
void version(void) {
  ...
  printf("Written by William McCarthy, edited by Nathan Yoon\n\n");
}
...
void suppress_common_lines(void) {
...
void init_options_files(int argc, const char* argv[]) {
  ...
  if (report_identical && !different) { printf("The files are identical.\n\n");   exit(0); } // FILES IDENTICAL MODE

  if (showbrief && different) { printf("The files are different.\n\n");   exit(0); }  // BRIEF MODE

  if (showsidebyside && showleftcolumn) {SidebySide_withLeft(); exit(0);} // SIDE by SIDE MODE WITH LEFT COLUMN

  if (showsidebyside && suppresscommon) {suppress_common_lines(); exit(0);} // SUPPRESS COMMON LINES MODE
}
